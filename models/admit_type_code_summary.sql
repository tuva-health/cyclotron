...
select
  ...
  case
    when (  max(valid_admit_type_code) = 1 and
            min(valid_admit_type_code) = 1
         ) then 1
    else 0
  ...
  case
    when (  max(valid_admit_type_code) = 1 and
            min(valid_admit_type_code) = 0
         ) then 1
    else 0
  end as valid_and_invalid_admit_type_code,
  
  case
    when (  max(valid_admit_type_code) = 0 and
            min(valid_admit_type_code) = 0
         ) then 1
    else 0
...
all_claim_ids as (
select claim_id from claims_with_missing_admit_type_code
union all
